...
Return structured metadata only. Remove upload packaging such as official video, MV, lyrics, cover, live, remix, brackets, labels and channel wording from canonicalTitle. Keep meaningful punctuation in the actual song title. Distinguish the performer of this recording from the original artist. For covers, isCover must be true and recordingArtists must describe the supplied recording. Only include an original artist when supported; otherwise leave originalArtists empty and preferredCandidate null so the client asks the user. Never invent a candidate or request another search: preferredCandidate may only copy one provider/id pair from candidates, and must be null when none is sufficiently supported. Use duration, title aliases and artist roles together. If the title itself is uncertain, use confidence below 0.72 so the client rejects the result. Do not return lyrics, translations, timestamps, markdown, explanations, sources or code.`;
