...
- **可选全屏 AI 导演（BYOK）**：扩展直接调用用户选择的 OpenAI Responses、OpenAI-compatible、本地模型、Gemini 或 Anthropic Messages API，不再依赖固定 LyricStage Director 服务器。模型只编排紧凑的整曲叙事意图与少量重点 cue，本地再确定性补齐逐行演出；整条主/修复/备用链共享 45 秒、最多 3 次 HTTP，同曲并发与迟到 MusicMap 不会重复生成。设置页会显示最近一次缓存、模型、合同与总耗时，但不记录 endpoint、Key、歌词或响应正文。失败最终回到完整确定性演出。
