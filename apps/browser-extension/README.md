...
扩展会在换歌后先按 `videoID` 读取已经采用的歌词绑定；命中后直接复用，不因标题、歌手、时长的小幅变化或缓存时间经过而重新联网。只有从未成功采用歌词的歌曲，才按 `videoID + 标题 + 歌手 + 时长` 进行分层查询；未命中与候选结果仍使用短期缓存：
...
- 已采用结果与该 `videoID` 持久绑定，最多保留 100 首；超过容量时按最近写入顺序淘汰旧绑定。用户主动选择其他歌词版本会覆盖同一歌曲的绑定。
