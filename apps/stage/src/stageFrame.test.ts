...
describe("StageFrameV1", () => {
  ...
  it("reconstructs the same ambient numbers after playback, seek, and hidden resume", () => {
    ...
    const direct = { ...writeStageFrameV1(directBuffers, input(targetTimeMs)).ambient };
    ...
    const playback = { ...writeStageFrameV1(playbackBuffers, input(targetTimeMs)).ambient };

    const resumeBuffers = createStageFrameBuffersV1(input(0));
    writeStageFrameV1(resumeBuffers, input(12_000));
    const resumed = { ...writeStageFrameV1(resumeBuffers, input(targetTimeMs)).ambient };
