...
describe("Apple-style YouTube Music shell contract", () => {
  ...
  it("keeps lyrics persistent and moves queue and related into toolbar popovers", () => {
    ...
    expect(contentScript).toContain('const APPLE_SHELL_MEDIA_TOGGLE_ATTR = "data-lyricstage-native-media-switch"');
    expect(contentScript).toContain('actions.append(mediaToggle)');
    ...
    expect(shellStyles).toContain('[data-lyricstage-native-media-switch="true"]');
