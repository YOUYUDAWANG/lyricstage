...
const clientDirectory = new URL("client/", dist);
const serverDirectory = new URL("server/", dist);
const metadataDirectory = new URL(".openai/", dist);
const showcaseDirectory = new URL("showcase/", clientDirectory);
...
await Promise.all([
  rm(new URL("index.html", dist), { force: true }),
  rm(new URL("assets/", dist), { recursive: true, force: true }),
  rm(new URL("showcase/", dist), { recursive: true, force: true }),
]);
